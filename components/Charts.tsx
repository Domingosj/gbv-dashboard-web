...
async function renderChart(el: HTMLElement, config: any) {
...
export function MonthlyChart({ cases }: MonthlyChartProps) {
  ...
  useEffect(() => {
    if (!ref.current || cases.length === 0) return;
    ...
    renderChart(ref.current, {
      chart: { type: "area", toolbar: { show: false }, height: 350 },
    ...
    }).then(chart => { chartRef.current = chart; });

    return () => { if (chartRef.current) chartRef.current.destroy(); };
  }, [cases]);

  if (cases.length === 0) return <p className="text-body text-sm px-4">Sem dados</p>;
  return <div ref={ref} className="-ml-3 mt-4" />;
}
...
export function ViolencePie({ cases }: ViolencePieProps) {
  ...
  return <div ref={ref} className="flex justify-center items-center py-6" />;
}
...
export function SimpleBarChart({ cases, field }: BarChartProps) {
  ...
  useEffect(() => {
    ...
    renderChart(ref.current, {
      ...
      colors: [CHART_COLORS[0]],
      plotOptions: { bar: { horizontal: false, columnWidth: "35%", borderRadius: 2 } },
      grid: { borderColor: "#E2E8F0", strokeDasharray: 5, xaxis: { lines: { show: false } }, yaxis: { lines: { show: true } } },
      dataLabels: { enabled: false },
      xaxis: { categories, axisBorder: { show: false }, axisTicks: { show: false }, labels: { style: { fontSize: "12px", colors: ["#64748B"] } } },
      yaxis: { labels: { style: { fontSize: "12px", colors: ["#64748B"] } } },
