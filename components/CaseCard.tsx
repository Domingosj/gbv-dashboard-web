...
export default function CaseCard({ case: c, index }: Props) {
  ...
  return (
    <div
      className={`genesis-card p-5 priority-${level.toLowerCase()} hover:shadow-card-hover hover:-translate-y-0.5`}
    >
      <div className="flex items-start justify-between mb-4">
        <div>
          <span className="font-display text-lg font-semibold text-text-primary">
            {config.icon} {violence}
          </span>
          <span className="ml-3 text-caption text-neutral bg-gray-100 px-2 py-0.5 rounded-full">
            Caso #{index}
          </span>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 text-small mb-4">
        {[
          ...
          <div key={label as string}>
            <p className="text-caption text-neutral">{label as string}</p>
            <p className="font-medium text-text-primary truncate">{value as string}</p>
          </div>
        ))}
      </div>
      ...
      {c.incident_description && (
        <details className="mb-4">
          <summary className="text-small text-primary cursor-pointer font-medium">
            📝 Descrição
          </summary>
      ...
      <div
        className={`text-small p-3 rounded-button font-medium ${
          ref.status === "CRITICO"
            ? "bg-red-50 text-critical"
            : ref.status === "ALTO" || ref.status === "SEM_REFERENCIA"
