  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Casos Ativos", value: stats.total, icon: Users, style: { border: "2px solid rgba(0, 82, 67, 0.2)", background: "rgba(0, 82, 67, 0.05)", color: "#005243" } },
          ...
          <div
            key={label}
            className="p-4 rounded-xl text-center hover:shadow-card transition-shadow"
            style={style}
          >
            <Icon className="w-6 h-6 mx-auto mb-2" style={{ color: style.color }} />
            <p className="text-body-sm text-on-surface-variant">{label}</p>
            <p className="text-metric font-bold" style={{ color: style.color }}>{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-surface-container-lowest rounded-lg border border-outline-variant p-4">
        <h3 className="text-title-md font-semibold mb-3 text-on-surface">Insights Ação</h3>
        <ul className="space-y-2">
          <li className="text-body-sm flex items-start gap-2 before:content-['•'] before:text-primary">
