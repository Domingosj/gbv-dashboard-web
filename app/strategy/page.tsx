...
function ResourcesTab({ cases }: { cases: GBVCase[] }) {
  const open = cases.filter(c => c.case_status === "Aberto");
  const distLoad: Record<string, { cases: number; critical: number; noRef: number }> = {};
  for (const c of open) {
    const d = c.district || "Desconhecido";
    if (!distLoad[d]) distLoad[d] = { cases: 0, critical: 0, noRef: 0 };
    distLoad[d].cases++;
    if (c.priority_level === "CRÍTICO") distLoad[d].critical++;
    if (!c.has_referral) distLoad[d].noRef++;
  }
  const highLoad = Object.entries(distLoad).filter(([, v]) => v.cases > 3).sort((a, b) => b[1].cases - a[1].cases).slice(0, 8);
  return (
...
function ReportingTab({ cases }: { cases: GBVCase[] }) {
  const provinces = Array.from(new Set(cases.map(c => c.province).filter((d): d is string => !!d))).length;
  const districts = Array.from(new Set(cases.map(c => c.district).filter((d): d is string => !!d))).length;
  const projects = Array.from(new Set(cases.map(c => c.project).filter((d): d is string => !!d))).length;
  const open = cases.filter(c => c.case_status === "Aberto");
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: "Total Casos", value: cases.length },
          { label: "Abertos", value: open.length },
          { label: "Encerrados", value: closed.length },
          { label: "Tx Encerramento", value: `${cases.length ? ((closed.length / cases.length) * 100).toFixed(1) : 0}%` },
        ].map(({ label, value }) => (
      ...
      <GCRCard title="Cobertura">
        <div className="grid grid-cols-3 gap-4 text-center">
          {[{ l: "Províncias", v: provinces }, { l: "Distritos", v: districts }, { l: "Projetos", v: projects }].map(({ l, v }) => (
            <div key={l}>
              <p className="text-metric text-primary">{v}</p>
              <p className="text-label text-text-secondary">{l}</p>
            </div>
          ))}
        </div>
      </GCRCard>
